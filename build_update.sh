...
(
  ...
  if [[ `git merge-base HEAD FETCH_HEAD` != `git rev-parse HEAD` ]]; then
    echo "OpenCV folder contains changes. Please save your changes and checkout the lastest code: $(pwd)"
    exit 1
  fi
  if ! git diff-index --quiet HEAD --; then
    echo "OpenCV folder contains unsaved changes. Please save (stash) your changes and checkout the lastest code: $(pwd)"
    exit 1
  fi
